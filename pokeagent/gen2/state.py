...
# The Gen-3 side owns the boundary schemas; Gen 2 validates through the same
# ones rather than carrying a second copy.
try:
    from ..schemas import validate_game_state
except ImportError:  # schemas is optional; a failed validation must not stop a read
    def validate_game_state(d, include_screen=False):
        return d
...
#: Set by the Gen-2 adapter to the pokecrystal checkout. The original derived
#: this from the tool's parent directory, which is exactly why that project
#: could not be cloned on its own.
DECOMP_ROOT = None
...
def _battle_constants():
    if DECOMP_ROOT is None:
        raise RuntimeError(
            "pokeagent.gen2.state.DECOMP_ROOT is unset; the Gen-2 adapter "
            "sets it to the pokecrystal checkout before any read"
        )
    return Path(DECOMP_ROOT) / "constants" / "battle_constants.asm"
@lru_cache(maxsize=1)
def _status_bits():
    """Lazily parsed, deliberately.

    The Crystal original computed this at MODULE SCOPE, so importing
    `crystalagent.state` required the disassembly on disk -- which meant every
    one of its ~580 pure unit tests failed to even collect without it. Keeping
    the parse behind a call is the fix.
    """
    return [(1 << parse_const_defs(_battle_constants())[n], n)
            for n in ("PSN", "BRN", "FRZ", "PAR")]


@lru_cache(maxsize=1)
def _slp_mask():
    return parse_defs(_battle_constants())["SLP_MASK"]
...
def _status(byte):
    ...
    if byte & _slp_mask():
        out.append(f"SLP:{byte & _slp_mask()}")
    return out
