...
class Crystal:
    ...
    def tilemap(self):
        return self.read(self._tilemap_symbol(), SCREEN_W * SCREEN_H)

    #: pokecrystal calls it wTilemap, pokered calls it wTileMap. One capital
    #: letter, and it is the difference between a decoded screen and a
    #: KeyError. Resolved once against the symbol table actually loaded, so a
    #: game whose name is neither fails loudly instead of reading address 0.
    _TILEMAP_NAMES = ("wTilemap", "wTileMap")

    def _tilemap_symbol(self):
        cached = getattr(self, "_tilemap_name", None)
        if cached:
            return cached
        for name in self._TILEMAP_NAMES:
            try:
                self.resolve(name)
            except KeyError:
                continue
            self._tilemap_name = name
            return name
        raise KeyError(
            f"no tilemap symbol in this build; tried {self._TILEMAP_NAMES}"
        )
