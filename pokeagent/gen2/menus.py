...
CURSOR = "▶"
#: $ec (1D static menus) and $ed (2D + scrolling menus) are the cursor bytes.
#: "▲" is here because pokered maps $ed TWICE -- to the town-map up arrow at
#: constants/charmap.asm:85 and to "▶" at :177 -- and the parser keeps the
#: first, so Red's perfectly ordinary NEW GAME cursor decodes as "▲". Same
#: byte, different alias; pokecrystal has no such collision ("▲" is $61
#: there). Matching all three finds the cursor in either game without
#: teaching the menu driver which game it is looking at.
CURSORS = "▷▶▲"
