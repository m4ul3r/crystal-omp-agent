...
class Hunt:
    def __init__(self, driver):
        ...
        self.encounters = {}
        self.spot = None
        # INSTALL IT ONCE. Every `goto`/`travel` that meets a wild goes
        # through `Driver.fight()` with no policy argument, and that reads
        # `battle_policy` -- so without this a walk-in encounter is settled
        # by tactics, which KOs the thing we came to register.
