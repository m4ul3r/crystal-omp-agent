...
def main(argv=None) -> int:
    ...
    # missing() returns DexEntry OBJECTS, not species ids. Comparing an int
    # against that set is always False, which made this check report every
    # map as owing nothing -- a broken pre-flight is worse than none, because
    # it silently skips real work. Project the entries onto species ids first.
    missing_ids = {e.species for e in target.missing()}
    owed = {d.names.species(r.species)
            for r in target.wild.for_map(_map)
            if r.species in missing_ids}
    if not owed:
